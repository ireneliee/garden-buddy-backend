...
def setup_data_routes(app):
    ...
    def submitTemperatureData():
        ...
        garden_data = DataService.storeTemperatureData(
            serial_id=identifier, air_temperature=value
        )
    ...
    def submitMoistureData():
        identifier = request.form.get("identifier")
        value = request.form.get("value")
        garden_data = DataService.storeMoistureData(
            serial_id=identifier, moisture=value
        )
        return jsonify(garden_data.serialize())
    ...
    def submitSalinityData():
        identifier = request.form.get("identifier")
        value = request.form.get("value")
        garden_data = DataService.storeSalinityData(
            serial_id=identifier, salinity=value
        )
        return jsonify(garden_data.serialize())
    ...
    def submitBrightnessData():
        identifier = request.form.get("identifier")
        value = request.form.get("value")
        garden_data = DataService.storeBrightnessData(
            serial_id=identifier, brightness=value
        )
        return jsonify(garden_data.serialize())
    ...
    @app.route("/data/getUserGardenData", methods=["GET"])
    def getUserGardenData():
        garden_id = request.args.get("garden_id")
        return DataService.getUserGardenData(garden_id=garden_id)
